"""Ranking for the expanded corpus. Reddit remains the primary evidence base."""
...
def score_area(area: dict[str, Any]) -> dict[str, Any]:
    ...
    store_corroboration = min(1.0, store_n / 12.0) if reddit_n >= 1 and store_n >= 1 else 0.0
...
def rank_areas(areas: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    if len(eligible) < MIN_RANKED_AREAS:
        leftover = [row for row in scored if not row["eligible"] and row["reddit_claim_count"] >= 1]
...
def rubric_payload(ranked: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "weights": dict(WEIGHTS),
        "note": (
            "Evidence strength is 70% Reddit + 15% store corroboration + 15% Groq share. "
            "Phase 5 expansion claims are Reddit claims and carry full weight; "
            "store-only areas are still not ranked."
        ),
        ...
        "ranked": [
            {
                ...
                "phase5_claims": row.get("phase5_claim_count"),
                "factors": row["factors"],
            }
            for row in ranked
        ],
